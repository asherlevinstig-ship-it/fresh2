...
// --- 6. TERRAIN GENERATION (WITH CLIENT_EDITS CHECK) ---
...
// --- 7. RENDER HELPERS (MESH CREATION) ---
...
function updateRigAnim(dt) {
  // Safe access to MESH.weaponRoot
  if (viewMode === 0 && MESH && MESH.weaponRoot) {
...
window.addEventListener("keydown", (e) => {
  ...
  if (e.code === "F6") {
    try {
      if (colyRoom) colyRoom.send("world:patch:req", { r: 160 });
